...
class ScreenChild extends StatelessWidget{
  ...
  String mandados){
    if(!(isNumber(ahorros)||isNumber(herencia)||isNumber(mensualidad)||isNumber(tiempoMens)||isNumber(mandados))){
      ...
      int acumulado = create.createChild(int.parse(ahorros), int.parse(herencia),
      int.parse(mensualidad)*int.parse(tiempoMens),int.parse(mandados));
      showAlert(context, "$acumulado");
      //Navigator.push(context, MaterialPageRoute(builder: (context)=>(create.edad>14)?ScreenAdult():ScreenFinal()));
    }
